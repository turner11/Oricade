export const WIDTH = 960
export const HEIGHT = 540
export const TARGET_FPS = 60
export const WALK_SPEED = 120

export const PLAYER_MAX_HP = 3
export const ATTACK_MS = 150
export const ATTACK_REACH = 24
export const IFRAME_MS = 1000
export const KNOCKBACK_SPEED = 220
export const KNOCKBACK_MS = 180
export const ZANE_HP = 3
export const ZANE_SPEED = 60
export const ZANE_DASH_SPEED = 200
export const SIGHT_RANGE = 160
export const TYPEWRITER_MS_PER_CHAR = 30

// ponytail: tuning knobs, not measured constants — eyeball these in the manual smoke test.
export const PHASE_MS = 30_000 // half-cycle; a full day+night is 60s
export const FOOTSTEP_MS = 300 // footstep repeat interval while moving
export const NIGHT_TINT = 0x0a1a3a
export const NIGHT_TINT_ALPHA = 0.45
export const ASH_SPEED_MULT = 1.5
export const PROJECTILE_SPEED = 140
export const PROJECTILE_MS = 2000
export const STORMY_FIRE_MS = 2500
export const WHISPER_SPREAD_RAD = 0.35
export const GALE_SPEED = 350
export const GALE_TURN_MS = 800
export const BOSS_HP_1 = 8
export const BOSS_HP_2 = 10
export const BOSS_FIRE_MS = 2000
export const DASH_SPEED = 420
export const DASH_MS = 180
export const DASH_COOLDOWN_MS = 800
export const CHARGE_MS = 500
export const CHARGED_DAMAGE = 2
export const CHARGED_SCALE = 1.8

export const STICK_DEADZONE = 0.25
export const TEXT_SPEED_MIN = 5
export const TEXT_SPEED_MAX = 100

...
export function createRuntime({ scene, playerBody }) {
  const artifactMesh = new THREE.Mesh(new THREE.IcosahedronGeometry(0.4), new THREE.MeshStandardMaterial({ color: PALETTE.accentGold }))
  ...
  for (const hazard of HAZARDS) {
    ...
    const mesh = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: PALETTE.accentPurple }),
    )
  ...
  return {
    update(inputP1, dt) {
      ...
      if (!wonBurstPlayed && checkTrapDungeonOutcome(playerBody.position, crouching, ARTIFACT_POSITION, HAZARDS) === 'win') {
        wonBurstPlayed = true
        spawnBurst(scene, ARTIFACT_POSITION, PALETTE.accentGold)
